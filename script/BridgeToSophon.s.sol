...
contract BridgeToSophonScript is Script {
    ...
    function run() public {
        ...
        console.log("Token name:", SOPH.name());
        console.log("Balance:", SOPH.balanceOf(msg.sender));
        uint256 allowanceSOPH = SOPH.allowance(msg.sender, SEPOLIA_CUSTOM_ERC20_SHARED_BRIDGE_L1);
        console.log("SOPH allowance:", allowanceSOPH);
        ...
        // Approve shared bridge to spend LINK tokens
        ...
        // uint256 L2_GAS_LIMIT = 50000; // TODO: with this value, it fails, why?
        uint256 TX_GAS_PER_PUBDATA_BYTE_LIMIT = 800;
        uint256 SOPHON_CHAIN_ID = 531050104; // Sophon Sepolia
        ...
        uint256 baseCost = IBridgehub(SEPOLIA_L1_BRIDGEHUB).l2TransactionBaseCost(
            SOPHON_CHAIN_ID, l2GasPrice, L2_GAS_LIMIT, TX_GAS_PER_PUBDATA_BYTE_LIMIT
        );
        ...
        bridge.requestL2TransactionTwoBridges( // No vale needed if base token is not ETH (e.g SOPH)
            L2TransactionRequestTwoBridgesOuter({
                ...
                chainId: SOPHON_CHAIN_ID,
                mintValue: 5e18, // base tokens (SOPH for Sophon Sepolia, ETH for Sepolia)
                // mintValue: baseCost, // TOOD: does not work because baseCost returns a very high number
                l2Value: 0,
